...
use gpui::{
    ...
    Pixels, Point, Style, View, VisualContext, Window,
};
...
struct PopoverMenuHandleState<M> {
    menu_builder: Rc<dyn Fn(&mut Window, &mut AppContext) -> Option<View<M>>>,
    ...
    pub fn is_focused(&self, window: &Window, cx: &AppContext) -> bool {
        self.0.borrow().as_ref().map_or(false, |state| {
            state
                ...
                .borrow()
                .as_ref()
                .map_or(false, |view| view.focus_handle(cx).is_focused(window, cx))
        })
    }
}
    ...
    child_builder: Option<
        Box<
            dyn FnOnce(
                    ...
                    Option<Rc<dyn Fn(&mut Window, &mut AppContext) -> Option<View<M>> + 'static>>,
                ) -> AnyElement
                + 'static,
        >,
    >,
    menu_builder: Option<Rc<dyn Fn(&mut Window, &mut AppContext) -> Option<View<M>> + 'static>>,
    anchor: Corner,
...
impl<M: ManagedView> PopoverMenu<M> {
    ...
    pub fn menu(
        mut self,
        f: impl Fn(&mut Window, &mut AppContext) -> Option<View<M>> + 'static,
    ) -> Self {
    ...
    fn resolved_offset(&self, window: &Window, cx: &AppContext) -> Point<Pixels> {
        self.offset.unwrap_or_else(|| {
            ...
            let offset = rems_from_px(5.) * window.rem_size(cx);
            match self.anchor {
...
fn show_menu<M: ManagedView>(
    builder: &Rc<dyn Fn(&mut Window, &mut AppContext) -> Option<View<M>>>,
    menu: &Rc<RefCell<Option<View<M>>>>,
    window: &mut Window,
    cx: &mut AppContext,
) {
...
impl<M: ManagedView> Element for PopoverMenu<M> {
    ...
    fn request_layout(
        ...
        window.with_element_state(
            global_id.unwrap(),
            cx,
            |element_state: Option<PopoverMenuElementState<M>>, window, cx| {
    ...
    fn prepaint(
        ...
        request_layout.child_layout_id.map(|layout_id| {
            let bounds = window.layout_bounds(layout_id, cx);
            window.with_element_state(global_id.unwrap(), cx, |element_state, _window, _cx| {
                let mut element_state: PopoverMenuElementState<M> = element_state.unwrap();
                element_state.child_bounds = Some(bounds);
                ((), element_state)
            });

            window.insert_hitbox(bounds, false, cx).id
        })
    }
        ...
        if let Some(mut menu) = request_layout.menu_element.take() {
            ...
            if let Some(child_hitbox) = *child_hitbox {
                ...
                window.on_mouse_event(cx, move |_: &MouseDownEvent, phase, window, cx| {
                    if phase == DispatchPhase::Bubble && child_hitbox.is_hovered(window, cx) {
