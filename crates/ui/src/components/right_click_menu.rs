...
use gpui::{
    ...
    LayoutId, ManagedView, MouseButton, MouseDownEvent, ParentElement, Pixels, Point, View,
...
pub struct RightClickMenu<M: ManagedView> {
    ...
    menu_builder: Option<Rc<dyn Fn(&mut Window, &mut AppContext) -> View<M> + 'static>>,
    anchor: Option<Corner>,
    attach: Option<Corner>,
}

impl<M: ManagedView> RightClickMenu<M> {
    pub fn menu(mut self, f: impl Fn(&mut Window, &mut AppContext) -> View<M> + 'static) -> Self {
    ...
    fn prepaint(
        ...
        PrepaintState {
            ...
            child_bounds: request_layout
                .child_layout_id
                .map(|layout_id| window.layout_bounds(layout_id, cx)),
        }
    }
            ...
            |this, element_state, window, cx| {
                ...
                window.on_mouse_event(cx, move |event: &MouseDownEvent, phase, window, cx| {
                    if phase == DispatchPhase::Bubble
                        ...
                        && hitbox_id.is_hovered(window, cx)
                    {
                        cx.stop_propagation();
