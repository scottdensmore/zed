...
impl IconSize {
    ...
    pub fn square_components(&self, window: &mut Window, cx: &mut AppContext) -> (Pixels, Pixels) {
        let icon_size = self.rems() * window.rem_size(cx);
        let padding = match self {
            IconSize::Indicator => DynamicSpacing::Base00.px(window, cx),
            IconSize::XSmall => DynamicSpacing::Base02.px(window, cx),
            IconSize::Small => DynamicSpacing::Base02.px(window, cx),
            IconSize::Medium => DynamicSpacing::Base02.px(window, cx),
        };
