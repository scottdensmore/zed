...
impl WindowControlStyle {
    pub fn default(_window: &Window, cx: &AppContext) -> Self {
    ...
    fn render(self, _window: &mut Window, _cx: &mut AppContext) -> impl IntoElement {
        ...
        h_flex()
            ...
            .on_click(move |_, window, cx| {
                cx.stop_propagation();
                match self.icon {
                    WindowControlType::Minimize => window.minimize_window(cx),
                    WindowControlType::Restore => window.zoom_window(cx),
                    WindowControlType::Maximize => window.zoom_window(cx),
