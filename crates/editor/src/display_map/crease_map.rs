...
impl<T> Crease<T> {
        ...
        TrailerElement: IntoElement,
    {
        Crease::Inline {
            range,
            placeholder,
            render_toggle: Some(Arc::new(move |row, folded, toggle, _window, cx| {
                // REFACTOR ERROR Parsing function signature let render_toggle: RenderToggle
                render_toggle(row, folded, toggle, cx).into_any_element()
            })),
            render_trailer: Some(Arc::new(move |row, folded, _window, cx| {
                // REFACTOR ERROR Parsing function signature let render_trailer: RenderTrailer
