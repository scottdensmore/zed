    ...
    pub fn story(&self, window: &mut Window, cx: &mut AppContext) -> AnyView {
        match self {
            Self::ApplicationMenu => window
                .new_view(cx, |cx| title_bar::ApplicationMenuStory::new(cx))
                .into(),
            Self::AutoHeightEditor => AutoHeightEditorStory::new(window, cx).into(),
            Self::Avatar => window.new_view(cx, |_| ui::AvatarStory).into(),
            Self::Button => window.new_view(cx, |_| ui::ButtonStory).into(),
            Self::CollabNotification => window
                .new_view(cx, |_| collab_ui::notifications::CollabNotificationStory)
                .into(),
            Self::ContextMenu => window.new_view(cx, |_| ui::ContextMenuStory).into(),
            Self::Cursor => window.new_view(cx, |_| crate::stories::CursorStory).into(),
            Self::DefaultColors => DefaultColorsStory::view(window, cx).into(),
            Self::Disclosure => window.new_view(cx, |_| ui::DisclosureStory).into(),
            Self::Focus => FocusStory::view(window, cx).into(),
            Self::Icon => window.new_view(cx, |_| ui::IconStory).into(),
            Self::IconButton => window.new_view(cx, |_| ui::IconButtonStory).into(),
            Self::Keybinding => window.new_view(cx, |_| ui::KeybindingStory).into(),
            Self::Label => window.new_view(cx, |_| ui::LabelStory).into(),
            Self::List => window.new_view(cx, |_| ui::ListStory).into(),
            Self::ListHeader => window.new_view(cx, |_| ui::ListHeaderStory).into(),
            Self::ListItem => window.new_view(cx, |_| ui::ListItemStory).into(),
            Self::OverflowScroll => window
                .new_view(cx, |_| crate::stories::OverflowScrollStory)
                .into(),
            Self::Picker => PickerStory::new(window, cx).into(),
            Self::Scroll => ScrollStory::view(window, cx).into(),
            Self::Tab => window.new_view(cx, |_| ui::TabStory).into(),
            Self::TabBar => window.new_view(cx, |_| ui::TabBarStory).into(),
            Self::Text => TextStory::view(window, cx).into(),
            Self::ToggleButton => window.new_view(cx, |_| ui::ToggleButtonStory).into(),
            Self::ToolStrip => window.new_view(cx, |_| ui::ToolStripStory).into(),
            Self::ViewportUnits => window
                .new_view(cx, |_| crate::stories::ViewportUnitsStory)
                .into(),
            Self::WithRemSize => window
                .new_view(cx, |_| crate::stories::WithRemSizeStory)
                .into(),
            Self::Vector => window.new_view(cx, |_| ui::VectorStory).into(),
        }
    }
}
