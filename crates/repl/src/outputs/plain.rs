...
pub fn text_style(window: &mut Window, cx: &mut AppContext) -> TextStyle {
    ...
    let text_style = TextStyle {
        ...
        line_height: window.line_height(cx).into(),
...
pub fn terminal_size(window: &mut Window, cx: &mut AppContext) -> terminal::TerminalSize {
    ...
    let font_pixels = text_style.font_size.to_pixels(window.rem_size(cx));
    ...
    let height = num_lines as f32 * window.line_height(cx);
...
impl TerminalOutput {
    ...
    pub fn append_text(&mut self, text: &str, _window: &mut Window, cx: &mut AppContext) {
