...
use gpui::{
    ...
    TextStyle, UTF16Selection, UnderlineStyle, View, WeakView, WhiteSpace, Window,
    WindowTextSystem,
};
...
impl LayoutRect {
    ...
    pub fn paint(
        ...
        window.paint_quad(fill(Bounds::new(position, size), self.color), cx);
    }
}
                ...
                //Layout current cell text
                {
                    if !is_blank(&cell) {
                        ...
                        let layout_cell = text_system
                            .shape_line(
                                cell_text.into(),
                                text_style.font_size.to_pixels(window.rem_size(cx)),
                                &[cell_style],
                            )
                            .unwrap();
    ...
    fn register_mouse_listeners(
        ...
        window.on_mouse_event(cx, {
            ...
            move |e: &MouseMoveEvent, phase, window, cx| {
                if phase != DispatchPhase::Bubble || !focus.is_focused(window, cx) {
                    return;
                }
...
impl Element for TerminalElement {
        ...
        self.interactivity.prepaint(
            ...
            |_, _, hitbox, window, cx| {
                ...
                let dimensions = {
                    ...
                    let font_id = window.text_system(cx).resolve_font(&text_style.font());
    ...
    fn paint(
        ...
        window.with_content_mask(Some(ContentMask { bounds }), cx, |window, cx| {
            ...
            window.paint_quad(fill(bounds, layout.background_color), cx);
            let origin =
            ...
            if self.can_navigate_to_selected_word && layout.last_hovered_word.is_some() {
                window.set_cursor_style(gpui::CursorStyle::PointingHand, &layout.hitbox, cx);
            } else {
                window.set_cursor_style(gpui::CursorStyle::IBeam, &layout.hitbox, cx);
            }
