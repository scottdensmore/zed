...
impl Interactivity {
    ...
    pub fn on_mouse_down_out(
        ...
        self.mouse_down_listeners
            .push(Box::new(move |event, phase, hitbox, window, cx| {
                if phase == DispatchPhase::Capture && !hitbox.contains(&window.mouse_position(cx)) {
                    (listener)(event, window, cx)
                }
            }));
    }
        ...
        T: 'static,
    {
        self.mouse_move_listeners
            .push(Box::new(move |event, phase, hitbox, window, cx| {
                if phase == DispatchPhase::Capture {
                    // REFACTOR ERROR Unexpected reference parents: field_expression reference_expression let_condition
...
pub trait InteractiveElement: Sized {
    ...
    fn drag_over<S: 'static>(
        ...
        self.interactivity().drag_over_styles.push((
            TypeId::of::<S>(),
            Box::new(move |currently_dragged: &dyn Any, _window, cx| {
                f(
                    StyleRefinement::default(),
                    currently_dragged.downcast_ref::<S>().unwrap(),
                    // REFACTOR ERROR Failed to parse signature: f: impl 'static + Fn(StyleRefinement, &S, &WindowContext<'_>) -> StyleRefinement
                    cx,
                )
            }),
        ));
    ...
    fn block_mouse_down(mut self) -> Self {
        self.on_mouse_down(MouseButton::Left, |_, _window, cx| cx.stop_propagation())
    }
}
...
impl Element for Div {
    ...
    fn prepaint(
        ...
        if let Some(handle) = self.interactivity.scroll_anchor.as_ref() {
            *handle.last_origin.borrow_mut() = bounds.origin - window.element_offset(cx);
        }
        let content_size = if request_layout.child_layout_ids.is_empty() {
            ...
            for (ix, child_layout_id) in request_layout.child_layout_ids.iter().enumerate() {
                let child_bounds = window.layout_bounds(*child_layout_id, cx);
        ...
        self.interactivity.prepaint(
            ...
            |_style, scroll_offset, hitbox, window, cx| {
                window.with_element_offset(scroll_offset, cx, |window, cx| {
...
impl Interactivity {
                    ...
                    if let Some(active_tooltip) = element_state.active_tooltip.as_ref() {
                        if let Some(active_tooltip) = active_tooltip.borrow().as_ref() {
                            if let Some(tooltip) = active_tooltip.tooltip.clone() {
                                self.tooltip_id = Some(window.set_tooltip(tooltip, cx));
                            }
                        }
                    }
                }
                ...
                window.with_text_style(
                    ...
                    |window, cx| {
                        window.with_content_mask(
                            style.overflow_mask(bounds, window.rem_size(cx)),
                            cx,
                            |window, cx| {
                                let hitbox = if self.should_insert_hitbox(&style) {
                                    Some(window.insert_hitbox(bounds, self.occlude_mouse, cx))
                                } else {
                                    None
                                };
    ...
    pub fn paint(
        ...
        self.hovered = hitbox.map(|hitbox| hitbox.is_hovered(window, cx));
        window.with_optional_element_state::<InteractiveElementState, _>(
            global_id,
            cx,
            |element_state, window, cx| {
                ...
                if let Some(debug_selector) = &self.debug_selector {
                    window.debug_bounds.insert(debug_selector.clone(), bounds);
                }
                ...
                window.with_element_opacity(style.opacity, cx, |window, cx| {
                    style.paint(
                        bounds,
                        window,
                        cx,
            ...
            let render_debug_text = |window: &mut Window, cx: &mut AppContext| {
                if let Some(text) = window
                    ...
                    .shape_text(
                        element_id.into(),
                        FONT_SIZE,
                        &[window.text_style(cx).to_run(str_len)],
                        None,
                    )
                    .ok()
                    .and_then(|mut text| text.pop())
                {
                    ...
                    if self.location.is_some()
                        && text_bounds.contains(&window.mouse_position(cx))
                        && window.modifiers(cx).secondary()
                    {
                        let secondary_held = window.modifiers(cx).secondary();
                        ...
                        let was_hovered = hitbox.is_hovered(window, cx);
                        window.on_mouse_event(cx, {
                            let hitbox = hitbox.clone();
                            move |_: &MouseMoveEvent, phase, window, cx| {
                                if phase == DispatchPhase::Capture {
                                    let hovered = hitbox.is_hovered(window, cx);
    ...
    fn paint_mouse_listeners(
        ...
        if let Some(focus_handle) = self.tracked_focus_handle.clone() {
            let hitbox = hitbox.clone();
            window.on_mouse_event(cx, move |_: &MouseDownEvent, phase, window, cx| {
                if phase == DispatchPhase::Bubble
                    && hitbox.is_hovered(window, cx)
                    && !window.default_prevented(cx)
                {
        ...
        for listener in self.mouse_down_listeners.drain(..) {
            let hitbox = hitbox.clone();
            window.on_mouse_event(cx, move |event: &MouseDownEvent, phase, window, cx| {
        ...
        for listener in self.mouse_up_listeners.drain(..) {
            let hitbox = hitbox.clone();
            window.on_mouse_event(cx, move |event: &MouseUpEvent, phase, window, cx| {
                listener(event, phase, &hitbox, window, cx);
            })
        }
        ...
        for listener in self.scroll_wheel_listeners.drain(..) {
            let hitbox = hitbox.clone();
            window.on_mouse_event(cx, move |event: &ScrollWheelEvent, phase, window, cx| {
                listener(event, phase, &hitbox, window, cx);
            })
        }
...
// REFACTOR ERROR Unexpected reference parents: field_expression field_expression call_expression
            || cx.active_drag.is_some() && !self.drag_over_styles.is_empty()
        {
            let hitbox = hitbox.clone();
            let was_hovered = hitbox.is_hovered(window, cx);
            window.on_mouse_event(cx, move |_: &MouseMoveEvent, phase, window, cx| {
                let hovered = hitbox.is_hovered(window, cx);
        ...
        if let Some(element_state) = element_state {
                                    ...
                                    > DRAG_THRESHOLD
                            {
                                if let Some((drag_value, drag_listener)) = drag_listener.take() {
                                    *clicked_state.borrow_mut() = ElementClickedState::default();
                                    ...
                                    // REFACTOR ERROR Unexpected reference parents: field_expression assignment_expression expression_statement
                ...
                window.on_mouse_event(cx, {
                    ...
                    move |event: &MouseUpEvent, phase, window, cx| match phase {
                        ...
                        DispatchPhase::Capture => {
                            let mut pending_mouse_down = pending_mouse_down.borrow_mut();
                            if pending_mouse_down.is_some() && hitbox.is_hovered(window, cx) {
            ...
            if let Some(tooltip_builder) = self.tooltip_builder.take() {
                ...
                window.on_mouse_event(cx, {
                    ...
                    move |_: &MouseMoveEvent, phase, window, cx| {
                        let is_hovered =
                            pending_mouse_down.borrow().is_none() && hitbox.is_hovered(window, cx);
                        let tooltip_is_hovered = tooltip_id
                            .map_or(false, |tooltip_id| tooltip_id.is_hovered(window, cx));
            ...
            if active_state.borrow().is_clicked() {
                ...
                let hitbox = hitbox.clone();
                window.on_mouse_event(cx, move |_: &MouseDownEvent, phase, window, cx| {
                    if phase == DispatchPhase::Bubble && !window.default_prevented(cx) {
                        let group_hovered = active_group_hitbox.map_or(false, |group_hitbox_id| {
                            group_hitbox_id.is_hovered(window, cx)
                        });
                        let element_hovered = hitbox.is_hovered(window, cx);
    ...
    fn paint_keyboard_listeners(&mut self, window: &mut Window, cx: &mut AppContext) {
        ...
        if let Some(context) = self.key_context.clone() {
            window.set_key_context(context, cx);
        }

        for listener in key_down_listeners {
            window.on_key_event(cx, move |event: &KeyDownEvent, phase, window, cx| {
                listener(event, phase, window, cx);
            })
        }

        for listener in key_up_listeners {
            window.on_key_event(cx, move |event: &KeyUpEvent, phase, window, cx| {
                listener(event, phase, window, cx);
            })
        }

        for listener in modifiers_changed_listeners {
            window.on_modifiers_changed(cx, move |event: &ModifiersChangedEvent, window, cx| {
                listener(event, window, cx);
            })
        }

        for (action_type, listener) in action_listeners {
            window.on_action(action_type, cx, listener)
        }
    }
    ...
    fn paint_hover_group_handler(&self, window: &mut Window, cx: &mut AppContext) {
        ...
        if let Some(group_hitbox) = group_hitbox {
            let was_hovered = group_hitbox.is_hovered(window, cx);
            window.on_mouse_event(cx, move |_: &MouseMoveEvent, phase, window, cx| {
                let hovered = group_hitbox.is_hovered(window, cx);
                if phase == DispatchPhase::Capture && hovered != was_hovered {
                    window.refresh(cx);
                }
            });
    ...
    pub fn compute_style(
        ...
        window.with_optional_element_state(global_id, cx, |element_state, window, cx| {
            let mut element_state =
                element_state.map(|element_state| element_state.unwrap_or_default());
            let style = self.compute_style_internal(hitbox, element_state.as_mut(), window, cx);
            (style, element_state)
            ...
            if !cx.has_active_drag() {
                if let Some(group_hover) = self.group_hover_style.as_ref() {
                    if let Some(group_hitbox_id) =
                        GroupHitboxes::get(&group_hover.group, window.deref_mut(cx))
                    {
                        if group_hitbox_id.is_hovered(window, cx) {
            ...
            if let Some(drag) = cx.active_drag.take() {
                ...
                if can_drop {
                    for (state_type, group_drag_style) in &self.group_drag_over_styles {
                        if let Some(group_hitbox_id) =
                            GroupHitboxes::get(&group_drag_style.group, window.deref_mut(cx))
                        {
...
impl ScrollAnchor {
    ...
    pub fn scroll_to(&self, window: &mut Window, cx: &mut AppContext) {
        ...
        window.on_next_frame(cx, move |_| {
