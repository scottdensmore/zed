...
use crate::{
    ...
    FocusHandle, LayoutId, Pixels, Point, Size, Style, ViewContext, Window, ELEMENT_ARENA,
};
...
trait ElementObject {
    ...
    fn request_layout(&mut self, _window: &mut Window, cx: &mut AppContext) -> LayoutId;

    fn prepaint(&mut self, _window: &mut Window, cx: &mut AppContext);

    fn paint(&mut self, _window: &mut Window, cx: &mut AppContext);
...
impl<E: Element> Drawable<E> {
        ...
        match mem::take(&mut self.phase) {
            ElementDrawPhase::Start => {
                let global_id = self.element.id().map(|element_id| {
                    window.push(element_id);
                    GlobalElementId(window.clone())
                });
    ...
    pub(crate) fn prepaint(&mut self, window: &mut Window, cx: &mut AppContext) {
        match mem::take(&mut self.phase) {
            ...
            | ElementDrawPhase::LayoutComputed {
                ...
                let node_id = window.dispatch_tree.push_node();
    ...
    pub(crate) fn paint(
        ...
        match mem::take(&mut self.phase) {
            ElementDrawPhase::Prepaint {
                ...
                window.dispatch_tree.set_active_node(node_id);
                self.element.paint(
                    global_id.as_ref(),
                    bounds,
                    &mut request_layout,
    ...
    pub(crate) fn layout_as_root(
        ...
        window.layout_bounds(layout_id, cx).size
    }
}
...
impl AnyElement {
    ...
    pub fn prepaint(&mut self, window: &mut Window, cx: &mut AppContext) -> Option<FocusHandle> {
        let focus_assigned = window.focus.is_some();

        self.0.prepaint(window, cx);

        if !focus_assigned {
            if let Some(focus_id) = window.focus {
                // REFACTOR ERROR Unexpected reference parents: field_expression reference_expression arguments
    ...
    pub fn prepaint_at(
        ...
        window.with_absolute_element_offset(origin, cx, |window, cx| self.prepaint(window, cx))
    }
